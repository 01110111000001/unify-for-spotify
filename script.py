...
class Container:
    ...
    def load_config(self):
        try:
            with open('config.json', 'r') as config_file:
                ...
                self.local_temp_download_folder = config['local_temp_download_folder']
                self.down_on_spot_exec_folder = config['down_on_spot_exec_folder']
    ...
    def get_playlist_name(self):
        self.playlist_name = self.session.user_playlist(
    ...
    def create_local_playlist_folder(self):
        self.local_playlist_folder = os.path.join(
            self.local_library_folder, self.playlist_name).strip()
    ...
    def get_spotify_tracks_raw(self):
        response = self.session.playlist_tracks(
        ...
        for folder, subfolders, files in os.walk(self.local_playlist_folder):
            for file in files:
                ...
                if file_ext in self.extensions_to_scan:
                    ...
                    music_file_mutagen = File(file_path)
                    ...
                    # music_tag somehow writes comment tag value twice, so we need to select first one
    ...
    def spotify_tracks_fix_save_as(self):
        ...
        for spotify_track in self.spotify_tracks_raw:
            ...
            spotify_track['save_as'] = re.sub(
                r'[\\/*?:"<>|]', "", f"{spotify_track['title']} - {spotify_track['artist']}")

            if title_artist in checked_titles_artists:
                save_as = re.sub(
                    r'[\\/*?:"<>|]', "", f"{spotify_track['title']} - {spotify_track['artist']} in {spotify_track['album']}")
        ...
        for local_track in self.local_tracks_raw:
            ...
            if local_track['track_id'] not in self.spotify_track_ids:
                ...
                if os.path.exists(local_track['file_path']):
                    ...
                    os.remove(local_track['file_path'])
                    ...
                    if local_track['file_name'] != spotify_track['save_as']:
                        ...
                        if os.path.exists(local_track['file_path']):
                            os.rename(local_track['file_path'], os.path.join(
                                local_track['file_dir'], spotify_track['save_as'] + local_track['file_ext']))
    ...
    def downloader(self):
        ...
        os.system(f"cd \"{self.down_on_spot_exec_folder}\" && down_on_spot -i \"{self.currently_downloading_track['track_url']}\" -o \"{self.local_temp_download_folder}/{self.currently_downloading_track['save_as']}.ogg\"")
    ...
    def edit_metadata(self):
        ...
        # forgot why i needed to edit them
    ...
    def move_downloaded_track(self):
        ...
        file_path_new = os.path.join(
            self.local_playlist_folder, self.newly_downloaded_track['file_name'] + self.newly_downloaded_track['file_ext'])
